...
void sofa1()
{
	...
	glColor3f(0.97, 0.62, 0.43);
	glPushMatrix();
	glTranslated(-0.367 + sofa_s, -0.280, 0 + sofa_d);			//rest
	glScaled(0.5, 1.5, 4);
	glutSolidCube(0.1);
	glPopMatrix();
}
...
void first()
{
	...
	glColor3f(0.75, 0.75, 0.75);
	strokeString(50, 850, 0.3, 0.3, "EAST WEST COLLEGE OF ENGINEERING", 4);
	strokeString(100, 750, 0.3, 0.3, "DEPARTMENT OF COMPUTER SCIENCE", 4);
	strokeString(300, 670, 0.3, 0.3, "AND ENGINEERING", 4);
	strokeString(200, 500, 0.3, 0.3, "COMPUTER GRAPHICS MINI PROJECT", 4);
	glColor3f(0, 0.5, 0.5);
	strokeString(360, 420, 0.4, 0.4, "3D HOUSE", 4);
	glColor3f(0.54, 0, 0);
	strokeString(50, 50, 0.2, 0.2, "Press (Enter) to display the house", 4);
	strokeString(650, 200, 0.2, 0.2, "By:", 4);
	strokeString(650, 60, 0.2, 0.2, "SWAPNA & VEDASHREE", 4);
    glColor3f(0.74,0.72,0.42);
	strokeString(650, 120, 0.2, 0.2, "1EE20CS059 & 1EE20CS060", 4);
	glutSwapBuffers();
}
